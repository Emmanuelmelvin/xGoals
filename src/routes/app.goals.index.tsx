...
function GoalsPage() {
  ...
  })}</section>{goalError ? <p className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">{goalError}</p> : null}{isGoalsLoading ? <p className="text-sm text-muted">Loading goals…</p> : visibleGoals.length === 0 ? (goals.length === 0 ? <section className="rounded-3xl border border-dashed border-line bg-wash px-6 py-16 text-center"><p className="mx-auto grid size-12 place-items-center rounded-2xl bg-blue-pale text-blue"><GoalIcon /></p><h3 className="mt-5 text-xl font-semibold tracking-[-0.04em]">No goals yet</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted">Create one clear outcome, then we’ll break down the permissions and workflows it needs.</p><button type="button" onClick={openCreateGoal} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-blue px-4 py-2.5 text-sm font-bold text-white"><PlusIcon />Create a goal</button></section> : <section className="rounded-3xl border border-dashed border-line bg-wash px-6 py-16 text-center"><h3 className="text-xl font-semibold tracking-[-0.04em]">No {activeLabel.toLowerCase()} yet</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-muted">Nothing matches this filter. Try another status or start a new goal.</p><section className="mt-6 flex flex-wrap justify-center gap-2"><button type="button" onClick={() => setFilter("all")} className="rounded-xl border border-line bg-white px-4 py-2.5 text-sm font-bold">Show all goals</button><button type="button" onClick={openCreateGoal} className="inline-flex items-center gap-2 rounded-xl bg-blue px-4 py-2.5 text-sm font-bold text-white"><PlusIcon />Create a goal</button></section></section>) : <ul className="grid gap-4 md:grid-cols-2">{visibleGoals.map((goal) => <GoalCard key={goal.id} goal={goal} />)}</ul>}</section></section>;
}
