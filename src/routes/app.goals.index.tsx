...
function GoalCard({ goal }: { goal: Goal }) { return <li id={`goal-${goal.id}`} className="rounded-3xl border border-line bg-white p-6 transition-colors hover:border-blue"><header className="flex items-start justify-between gap-4"><h3 className="text-xl font-semibold tracking-[-0.05em]">{goal.title}</h3><span className="flex shrink-0 items-center gap-0.5"><GoalCardActions goal={goal} /><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-blue-pale text-blue"><GoalIcon /></span></span></header><footer className="mt-8 pt-4"><GoalCardMeta goal={goal} /></footer></li>; }
